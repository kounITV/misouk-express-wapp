...
export const EnhancedActionsDropdown: React.FC<EnhancedActionsDropdownProps> = ({
  ...
  // Dynamic positioning state - always left
    ...
    if (isOpen && buttonRef.current) {
      ...
      setTimeout(() => {
        if (buttonRef.current) {
          ...
          console.log('Button position:', buttonRect.right, 'Viewport width:', viewportWidth);
          console.log('Would overflow right:', buttonRect.right + dropdownWidth > viewportWidth - margin);
          
          // Always position to the left of the button
          let left = buttonRect.left - dropdownWidth;
          let right = 'auto';
          
          console.log('Positioning to LEFT at:', left);
          
          // Ensure dropdown doesn't go off the left edge
          if (left < margin) {
            left = margin;
            console.log('Adjusted to margin:', left);
          }
          
          setDropdownStyle({
            position: 'fixed',
            top: isLastItems ? buttonRect.top - 8 : buttonRect.bottom + 8,
            left: left,
            right: right,
            zIndex: 9999,
            width: dropdownWidth
  ...
  const handleStatusSelection = (status: string, label: string) => {
    ...
    if (hasLeftThaiBranch && userRole !== 'super_admin' && userRole !== 'lao_admin') {
    ...
    if (userRole === 'lao_admin' && currentStatus && status) {
      if (isRollback(currentStatus, status)) {
            ...
            >
              <div className="py-1">
                ...
                >
                  ...
                  {showStatusSubmenu && (
                    <div 
                      className="absolute w-48 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5"
                      style={{
                        right: '100%',
                        top: '0px',
                        marginRight: '4px',
